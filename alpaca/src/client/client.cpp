...
httplib::Headers headers(const Environment& environment) {
    return {{"APCA-API-KEY-ID", environment.getAPIKeyID()},
...
std::string Client::getSymbolsFromVector(const std::vector<std::string>& symbols) const{
...
std::pair<Status, PortfolioHistory> Client::getPortfolioHistory(const std::string& period,
    ...
    DLOG(INFO) << "Maiking request to: " << url;
    httplib::SSLClient client(environment_.getAPIBaseURL());
    auto resp = client.Get(url.c_str(), headers(environment_));
    if (!resp) {
        std::ostringstream ss;
...
std::pair<Status, Bars> Client::getStockBars(const std::vector<std::string>& symbols,
    ...
    std::string symbols_string = getSymbolsFromVector(symbols);
...
std::pair<Status, Bars> Client::getCryptoBars(const std::vector<std::string>& symbols,
    ...
    auto url = "/"CRYPTO_VERSION"/crypto/us/bars?" + query_string;
    httplib::SSLClient client(environment_.getAPIDataURL());
    DLOG(INFO) << "Making request to: " << url;
    auto resp = client.Get(url.c_str(), headers(environment_));
    if (!resp) {
...
std::pair<Status, LastOrderBooks> Client::getCryptoLastOrderBook(const std::vector<std::string>& symbols) const {
    ...
    std::string symbols_list = getSymbolsFromVector(symbols);
    // Create query string
    httplib::Params params{{"symbols", symbols_list}};

    auto query_string = httplib::detail::params_to_query_str(params);
    auto url = "/"CRYPTO_VERSION"/crypto/us/latest/orderbooks?" + query_string;
    httplib::SSLClient client(environment_.getAPIDataURL());
    DLOG(INFO) << "Making request to " << environment_.getAPIDataURL() << url;
    auto resp = client.Get(url.c_str(), headers(environment_));
    if (!resp) {
        std::ostringstream ss;
        ss << "Call to " << url << "returned an empty response";
...
// std::pair<Status, LastOrderBook> Client::getCryptoStreamedOrderbooks(
//         const std::vector<std::string>& symbols_trades,
//         const std::vector<std::string>& symbols_quotes,
//         const std::vector<std::string>& symbols_bars,
//         const void (*callback)(std::string& json)
//     ) const{

//     //Connect and authenticate to stream server

//     //TODO: Unsubscribe to all current subscriptions (Maybe a future sol. would be to maintain a map of strategies
//     // that need symbols, and unsub all symbols that don't have a strategy that needs them)
